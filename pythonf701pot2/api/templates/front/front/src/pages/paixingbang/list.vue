...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>排行：</div>
        <el-input v-model="formSearch.paixingstart" placeholder="最小排行" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
        <el-input v-model="formSearch.paixingend" placeholder="最大排行" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('paixingbang','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 10px","margin":"0px 10px 0 0","color":"#333","borderRadius":"2px","background":"#0062a530","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/paixingbangAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/xinwenxinxi/xinwenbiaoti').then(res => {
			if (res.data.code == 0) {
				this.xinwenbiaotiOptions = res.data.data;
				this.selectOptionsList.push({name:'新闻标题',list:this.xinwenbiaotiOptions,tableName: 'xinwenbiaoti',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if(this.formSearch.paixingstart!='' && this.formSearch.paixingstart!=undefined ){
          searchWhere.paixingstart = this.formSearch.paixingstart
        }
        if(this.formSearch.paixingend!='' && this.formSearch.paixingend!=undefined){
          searchWhere.paixingend = this.formSearch.paixingend
        }
