...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>新闻标题：</div>
        <el-input v-model="formSearch.xinwenbiaoti" placeholder="新闻标题" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('xinwenxinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ccc","padding":"0px 10px","margin":"0px 10px 0 0","color":"#333","borderRadius":"2px","background":"#0062a530","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/xinwenxinxiAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0 0 10px","margin":"10px 0 0","borderColor":"#cbcbcb","background":"#fff","borderWidth":"0 0 1px","display":"flex","width":"100%","fontSize":"0","position":"relative","borderStyle":"dotted","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div :style='{"border":"0px solid #ddd","width":"160px","padding":"0px","overflow":"hidden","height":"100px"}'>
					<img @click.stop="imgPreView(item.xinwenfengmian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.xinwenfengmian && item.xinwenfengmian.substr(0,4)=='http'" :src="item.xinwenfengmian.split(',')[0]" class="image" />
					<img @click.stop="imgPreView(baseUrl + (item.xinwenfengmian?item.xinwenfengmian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.xinwenfengmian?item.xinwenfengmian.split(',')[0]:'')" class="image" />
				</div>
				<div class="item-info" :style='{"padding":"10px","overflow":"hidden","alignItems":"center","color":"#666","flexWrap":"wrap","flex":"1","background":"none","display":"flex","fontSize":"14px","height":"auto"}'>
					<div :style='{"border":"0px solid #eee","padding":"0","margin":"0 20px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","overflow":"hidden","background":"none","width":"auto","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600"}' class="name ">{{item.xinwenbiaoti}}</div>
	...
	<!-- 热门信息 -->
	<div class="hot" :style='{"width":"100%","padding":"0","margin":"0 0 20px","background":"none","height":"auto","order":"90"}'>
	  <div :style='{"padding":"20px 0px","borderColor":"#eccc19","color":"#3a576a","borderRadius":"0px","textAlign":"center","background":"url(http://codegen.caihongy.cn/20240117/e2da3af842f44f9f86c61d0ff295414d.png) no-repeat center center / 35%","borderWidth":"0 0 0px","width":"100%","fontSize":"24px","borderStyle":"solid","fontWeight":"500"}'>推荐信息</div>
	  <div :style='{"padding":"0","margin":"10px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
	    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","width":"23%","margin":"0 0 20px","borderRadius":"8px","background":"#fff","height":"auto"}' @click="toDetail(item)">
	      <img :style='{"width":"48%","objectFit":"cover","borderRadius":"4px","float":"left","display":"block","height":"140px"}' v-if="item.xinwenfengmian && item.xinwenfengmian.substr(0,4)=='http'" :src="item.xinwenfengmian.split(',')[0]" alt="">
	      <img :style='{"width":"48%","objectFit":"cover","borderRadius":"4px","float":"left","display":"block","height":"140px"}' v-else :src="baseUrl + (item.xinwenfengmian?item.xinwenfengmian.split(',')[0]:'')" alt="">
	      <div :style='{"padding":"0 10px","borderColor":"#eee","whiteSpace":"nowrap","color":"#666","textAlign":"left","float":"right","overflow":"hidden","borderWidth":"0 0 0px","width":"48%","lineHeight":"32px","fontSize":"16px","textOverflow":"ellipsis","borderStyle":"solid","fontWeight":"600"}'>{{item.xinwenbiaoti}}</div>
	      <!--<div :style='{"padding":"0 10px","color":"#999","textAlign":"left","width":"48%","lineHeight":"32px","fontSize":"14px","float":"right"}'>2022-02-02</div>-->
	    </div>
	  </div>
	</div>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/xinwenbiaoqian/xinwenbiaoqian').then(res => {
			if (res.data.code == 0) {
				this.xinwenbiaoqianOptions = res.data.data;
				this.selectOptionsList.push({name:'新闻标签',list:this.xinwenbiaoqianOptions,tableName: 'xinwenbiaoqian',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getHotList() {
        let autoSortUrl = "";
        autoSortUrl = "xinwenxinxi/autoSort";
        if(localStorage.getItem('frontToken')) {
            autoSortUrl = "xinwenxinxi/autoSort2";
        }
      ...
      getFenlei() {
		this.$http.get('xinwenleixing/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'xinwenleixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.xinwenbiaoti != '') searchWhere.xinwenbiaoti = '%' + this.formSearch.xinwenbiaoti + '%';
        if (this.formSearch.xinwenbiaoqian != '') searchWhere.xinwenbiaoqian = this.formSearch.xinwenbiaoqian;
        if (this.curFenlei != '全部') searchWhere.xinwenleixing = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`xinwenxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/xinwenxinxiDetail', query: params});
      },
